...
func (b *FileUploadAndDownloadApi) BreakpointContinue(ctx context.Context, c *app.RequestContext) {
	...
	cen, _ := io.ReadAll(f)
	if !utils.CheckMd5(cen, string(chunkMd5)) {
		global.LOG.Error("检查md5失败!", zap.Error(err))
		response.FailWithMessage("检查md5失败", c)
		return
	}
	file, err := fileUploadAndDownloadService.FindOrCreateFile(string(fileMd5), string(fileName), chunkTotal)
	if err != nil {
		global.LOG.Error("查找或创建记录失败!", zap.Error(err))
		response.FailWithMessage("查找或创建记录失败", c)
		return
	}
	pathC, err := utils.BreakPointContinue(cen, string(fileName), chunkNumber, chunkTotal, string(fileMd5))
	if err != nil {
