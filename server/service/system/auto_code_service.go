...
import (
	...
	"github.com/edufriendchen/hertz-vue-admin/server/resource/autocode_template/subcontract"
...
const (
	...
	packageAPI         = "controller/v1/%s/enter.go"
	packageAPIName     = "controller/v1"
)
...
func Init(Package string) {
	...
	packageInjectionMap = map[string]astInjectionMeta{
		packageServiceName: {
			...
			importCodeF:  "github.com/edufriendchen/hertz-vue-admin/server/%s/%s",
			packageNameF: "%s",
		...
		packageAPIName: {
			path: filepath.Join(global.CONFIG.AutoCode.Root,
				global.CONFIG.AutoCode.Server, "controller/v1", "enter.go"),
			importCodeF:  "github.com/edufriendchen/hertz-vue-admin/server/%s/%s",
			packageNameF: "%s",
			groupName:    "ApiGroup",
...
func (autoCodeService *AutoCodeService) CreateTemp(autoCode system.AutoCodeStruct, ids ...uint) (err error) {
	...
	if autoCode.AutoMoveFile { // 判断是否需要自动转移
		...
		var routePath = filepath.Join(global.CONFIG.AutoCode.Root,
			global.CONFIG.AutoCode.Server, global.CONFIG.AutoCode.SInitialize, "router.go")
		var imporStr = fmt.Sprintf("github.com/edufriendchen/hertz-vue-admin/server/model/%s", autoCode.Package)
...
func (autoCodeService *AutoCodeService) addAutoMoveFile(data *tplData) {
	...
	if strings.Contains(fileSlice[1], "server") {
		...
		} else if strings.Contains(fileSlice[n-2], "controller") {
...
func (autoCodeService *AutoCodeService) getNeedList(autoCode *system.AutoCodeStruct) (dataList []tplData, fileList []string, needMkdir []string, err error) {
	...
	// resource/template/web/controller.js.tpl -> autoCode/web/autoCode.PackageName/controller/autoCode.PackageName.js
